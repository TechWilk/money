...
use TechWilk\Money\LoginFailure;
use TechWilk\Money\LoginFailureQuery;
...
class LoginFailureTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'TechWilk.Money.Map.LoginFailureTableMap';
     ...
     */
    const OM_CLASS = '\\TechWilk\\Money\\LoginFailure';

    /**
     * A class that can be returned by this tableMap.
     */
    const CLASS_DEFAULT = 'TechWilk.Money.LoginFailure';
    ...
    protected static $fieldNames = [
        self::TYPE_PHPNAME       => ['Username', 'ipAddress', 'Timestamp'],
        self::TYPE_CAMELNAME     => ['username', 'ipAddress', 'timestamp'],
        self::TYPE_COLNAME       => [self::COL_USERNAME, self::COL_IPADDRESS, self::COL_TIMESTAMP],
        self::TYPE_FIELDNAME     => ['username', 'ipAddress', 'timestamp'],
        self::TYPE_NUM           => [0, 1, 2],
    ];
    ...
    protected static $fieldKeys = [
        self::TYPE_PHPNAME       => ['Username' => 0, 'ipAddress' => 1, 'Timestamp' => 2],
        self::TYPE_CAMELNAME     => ['username' => 0, 'ipAddress' => 1, 'timestamp' => 2],
        self::TYPE_COLNAME       => [self::COL_USERNAME => 0, self::COL_IPADDRESS => 1, self::COL_TIMESTAMP => 2],
        self::TYPE_FIELDNAME     => ['username' => 0, 'ipAddress' => 1, 'timestamp' => 2],
        self::TYPE_NUM           => [0, 1, 2],
    ];
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\TechWilk\\Money\\LoginFailure');
 ...
 // buildRelations()
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(self::COL_USERNAME);
            $criteria->addSelectColumn(self::COL_IPADDRESS);
            $criteria->addSelectColumn(self::COL_TIMESTAMP);
        } else {
    ...
    public static function buildTableMap()
    {
     ...
     public static function doDelete($values, ConnectionInterface $con = null)
     {
         ...
         if ($values instanceof Criteria) {
             // rename for clarity
            $criteria = $values;
         } elseif ($values instanceof \TechWilk\Money\LoginFailure) { // it's a model object
