...
abstract class TransactionQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \TechWilk\Money\Base\TransactionQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'money', $modelName = '\\TechWilk\\Money\\Transaction', $modelAlias = null)
    {
    ...
    public function filterByValue($value = null, $comparison = null)
    {
        if (is_array($value)) {
            $useMinMax = false;
            if (isset($value['min'])) {
                $this->addUsingAlias(TransactionTableMap::COL_VALUE, $value['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($value['max'])) {
                $this->addUsingAlias(TransactionTableMap::COL_VALUE, $value['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(TransactionTableMap::COL_VALUE, $value, $comparison);
    }
    ...
    /**
     * Filter the query by a related \TechWilk\Money\Breakdown object
     *
     * @param \TechWilk\Money\Breakdown|ObjectCollection $breakdown the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildTransactionQuery The current query, for fluid interface
     */
    public function filterByBreakdown($breakdown, $comparison = null)
    {
        if ($breakdown instanceof \TechWilk\Money\Breakdown) {
            return $this
                ->addUsingAlias(TransactionTableMap::COL_ID, $breakdown->getTransactionId(), $comparison);
        } elseif ($breakdown instanceof ObjectCollection) {
            return $this
                ->useBreakdownQuery()
                ->filterByPrimaryKeys($breakdown->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByBreakdown() only accepts arguments of type \TechWilk\Money\Breakdown or Collection');
        }
    }
     ...
     * @return $this|ChildTransactionQuery The current query, for fluid interface
     */
    public function joinBreakdown($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('Breakdown');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'Breakdown');
        }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \TechWilk\Money\BreakdownQuery A secondary query class using the current class as primary query
     */
    public function useBreakdownQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinBreakdown($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Breakdown', '\TechWilk\Money\BreakdownQuery');
    }

    /**
     * Filter the query by a related \TechWilk\Money\TransactionHashtag object
     *
     * @param \TechWilk\Money\TransactionHashtag|ObjectCollection $transactionHashtag the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildTransactionQuery The current query, for fluid interface
     */
    ...
    {
        if ($transactionHashtag instanceof \TechWilk\Money\TransactionHashtag) {
            return $this
            ...
            throw new PropelException('filterByTransactionHashtag() only accepts arguments of type \TechWilk\Money\TransactionHashtag or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \TechWilk\Money\TransactionHashtagQuery A secondary query class using the current class as primary query
     */
    public function useTransactionHashtagQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ...
            ->useQuery($relationAlias ? $relationAlias : 'TransactionHashtag', '\TechWilk\Money\TransactionHashtagQuery');
    }
