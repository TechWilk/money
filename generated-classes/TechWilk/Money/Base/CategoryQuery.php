...
use TechWilk\Money\Map\CategoryTableMap;
 ...
 * @method     \TechWilk\Money\AccountQuery|\TechWilk\Money\BreakdownQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
...
abstract class CategoryQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \TechWilk\Money\Base\CategoryQuery object.
     *
     * @param string $dbName     The database name
     * @param string $modelName  The phpName of a model, e.g. 'Book'
     * @param string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'money', $modelName = '\\TechWilk\\Money\\Category', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, name, account_id FROM category WHERE id = :p0';
        try {
    ...
    public function filterByAccountId($accountId = null, $comparison = null)
    {
        if (is_array($accountId)) {
            ...
            if (isset($accountId['min'])) {
                $this->addUsingAlias(CategoryTableMap::COL_ACCOUNT_ID, $accountId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($accountId['max'])) {
                $this->addUsingAlias(CategoryTableMap::COL_ACCOUNT_ID, $accountId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(CategoryTableMap::COL_ACCOUNT_ID, $accountId, $comparison);
    }
    ...
    public function filterByAccount($account, $comparison = null)
    {
        if ($account instanceof \TechWilk\Money\Account) {
            return $this
                ->addUsingAlias(CategoryTableMap::COL_ACCOUNT_ID, $account->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(CategoryTableMap::COL_ACCOUNT_ID, $account->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
    ...
    public function filterByBreakdown($breakdown, $comparison = null)
    {
        if ($breakdown instanceof \TechWilk\Money\Breakdown) {
            return $this
                ->addUsingAlias(CategoryTableMap::COL_ID, $breakdown->getCategoryId(), $comparison);
